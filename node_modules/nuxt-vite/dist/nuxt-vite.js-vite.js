...
function cacheDirPlugin(rootDir, name) {
  ...
  return {
    ...
    async configResolved(resolvedConfig) {
      ...
      await fsExtra.mkdirp(upath.resolve(optimizeCacheDir, "temp"));
    }
  };
}
...
async function buildClient(ctx) {
  ...
  for (const p of ctx.builder.plugins) {
    alias[p.name] = p.mode === "server" ? `defaultexport:${require$$1.resolve(ctx.nuxt.options.buildDir, "empty.js")}` : `defaultexport:${p.src}`;
  }
  const clientConfig = vite.mergeConfig(ctx.config, {
    ...
    build: {
      ...
      assetsDir: ".",
      rollupOptions: {
        input: require$$1.resolve(ctx.nuxt.options.buildDir, "client.js")
      }
    },
    plugins: [
...
async function buildServer(ctx) {
  ...
  const alias = {};
  for (const p of ctx.builder.plugins) {
    alias[p.name] = p.mode === "client" ? `defaultexport:${require$$1.resolve(ctx.nuxt.options.buildDir, "empty.js")}` : `defaultexport:${p.src}`;
  }
  const serverConfig = vite.mergeConfig(ctx.config, {
    ...
    build: {
      ...
      ssr: true,
      rollupOptions: {
        input: require$$1.resolve(ctx.nuxt.options.buildDir, "server.js"),
  ...
  const serverDist = require$$1.resolve(ctx.nuxt.options.buildDir, "dist/server");
  await fsExtra.mkdirp(serverDist);
  await fsExtra.writeFile(require$$1.resolve(serverDist, "index.ssr.html"), APP_TEMPLATE);
  await fsExtra.writeFile(require$$1.resolve(serverDist, "index.spa.html"), APP_TEMPLATE);
  await fsExtra.writeFile(require$$1.resolve(serverDist, "client.manifest.json"), JSON.stringify({
    publicPath: "",
    all: [],
    initial: [
      "client.js"
  ...
  await fsExtra.writeFile(require$$1.resolve(serverDist, "server.manifest.json"), JSON.stringify({
    entry: "server.js",
    files: {
      "server.js": "server.js"
    },
  ...
  const onBuild = debounce__default['default'](async () => {
...
async function bundle(nuxt, builder) {
  for (const p of builder.plugins) {
    p.src = nuxt.resolver.resolvePath(require$$1.resolve(nuxt.options.buildDir, p.src));
  }
  const ctx = {
    nuxt,
    builder,
